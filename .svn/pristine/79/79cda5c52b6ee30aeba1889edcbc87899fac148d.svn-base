...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private void plotRoutes() {
        ...
        if (s != null) {
            for (Route aRoute : s.getRoutes()) {
                busRouteLegendOverlay.add(aRoute.getNumber());
                for (RoutePattern routePattern : aRoute.getPatterns()) {
                    LatLon lastPoint = null;
                    for (LatLon location : routePattern.getPath()) {
                        if (lastPoint != null && Geometry.rectangleIntersectsLine(northWest,southEast,lastPoint,location)) {
                            Polyline polyline = new Polyline(mapView.getContext());
                            polyline.setColor(busRouteLegendOverlay.getColor(aRoute.getNumber()));
                            polyline.setWidth(getLineWidth(zoomLevel));
                            polyline.setVisible(true);
                            List<GeoPoint> points = new ArrayList<GeoPoint>();

                            points.add(new GeoPoint(lastPoint.getLatitude(),lastPoint.getLongitude()));
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
    ...
    private void handleLocationChange(Location location) {
        LatLon userLocation = new LatLon(location.getLatitude(),location.getLongitude());
        Stop nearestStop = StopManager.getInstance().findNearestTo(userLocation);

        updateMarkerOfNearest(nearestStop);
        locationListener.onLocationChanged(nearestStop,userLocation);
    }
